...
#define H1 if (level >= X265_LOG_DEBUG) printf
...
#endif
        H0("   --dolby-vision-profile <float|integer> Specifies Dolby Vision profile ID. Currently, only profile 5, profile 8.1 and profile 8.2 are enabled. Specified as '5' or '50'. Default 0 (disabled).\n");
        ...
        H0("   --[no-]copy-pic               Copy buffers of input picture in the frame. Default %s\n", OPT(param->bCopyPicToFrame));
        ...
        H0("   --[no-]high-tier              If a decoder level is specified, this modifier selects the High tier of that level\n");
        ...
        H0("                                 f~ffffffffffffffffff(18), e~eeeeeeeeeeeee(13), (sane range is e3~f7),\n");
        H0("                                 vq1~vq3(vq1 if no trailing number(i.e. --tune vq == --tune vq1)),\n");
        ...
        H0("   --rskip <Integer>             Enable recursion skip for an early exit from CTU analysis during inter-prediction.\n"
           "                                     - 0: disabled.\n"
           "                                     - 1: exit using RD cost & CU homogeneity.\n"
           "                                     - 2: exit using CU edge density.\n"
           "                                 Default %d\n", param->recursionSkipMode);
        H1("   --rskip-edge-threshold        Threshold in terms of percentage (an integer of range [0,100]) for minimum edge density in CU's used to prune the recursion depth.\n");
        H1("                                 Applicable only to rskip mode 2. Value is preset dependent. Default: %.f\n", param->edgeVarThreshold*100.0f);
        ...
        H0("   --ctu-info <integer>          Enable receiving ctu information asynchronously and determine the reaction to the CTU information (0, 1, 2, 4, 6) Default 0\n"
        ...
        H0("   --cra-nal                     Force nal type to CRA to all frames except the first frame, works only with keyint 1. Default %s\n", OPT(param->craNal));
        ...
        H0("   --pass                        Multi pass rate control.\n"
            ...
            "                                   - 2 : Last pass, does not overwrite the stats file\n"
            "                                   - 3 : Nth pass, overwrites the stats file\n");
        ...
        H0("   --[no-]vbv-live-multi-pass    Enable real-time VBV in rate control 2 pass. Default %s\n", OPT(param->bliveVBV2pass));
        ...
        H0("   --scale-factor <int>          Specify the factor by which the input video is scaled down for analysis save mode. Default %d\n", param->scaleFactor);
        ...
        H0("   --max-ausize-factor <float>   This value controls the maximum AU size defined in the specification.\n");
        ...
        H0("   --[no-]opt-ref-list-length-pps  Dynamically set L0 and L1 ref list length in PPS (instead of default 0) based on values in the last GOP. Default %s\n", OPT(param->bOptRefListLengthPPS));
        H0("   --[no-]multi-pass-opt-rps     Enable storing commonly used RPS in SPS in multi-pass mode. Default %s\n", OPT(param->bMultiPassOptRPS));
        H0("   --[no-]opt-cu-delta-qp        Optimize to signal consistent CU level delta QPs in the frame. Default %s\n", OPT(param->bOptCUDeltaQP));
        ...
        H0("   --format                      Format of the input video\n"
           "                                   - 0 : normal\n"
           "                                   - 1 : side-by-side\n"
           "                                   - 2 : over-under\n"
           "                                 Default %d\n", param->format);
        ...
        H0("   --scc <integer>               Enable screen content coding\n"
           "                                  - 0: Disabled\n"
           "                                  - 1: Intrablockcopy fast search with 1x2 CTUs search range\n"
           "                                  - 2: Intrablockcopy Full search\n"
           "                                 Default %d\n", param->bEnableSCC);
#endif
#ifdef SVT_HEVC
        ...
        H0("   --svt-base-layer-switch-mode  Select whether B/P slice should be used in base layer for SVT-HEVC encoder\n"
           "                                  - 0: Use B-frames\n"
